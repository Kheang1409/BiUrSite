...
public class GitHubImageStorageService : IImageStorageService
{
    ...
    public async Task<string> UploadImageAsync(string fileName, byte[] content)
    {
        ...
        var url = $"https://api.github.com/repos/{_username}/{_repo}/contents/{fileName}";
        var response = await client.PutAsync(url, body);

        if (!response.IsSuccessStatusCode)
            throw new Exception("GitHub upload failed");
        return $"https://github.com/{_username}/{_repo}/blob/main/{fileName}?raw=true";
    }
    ...
    public async Task DeleteImageAsync(string fileName)
    {
        ...
        if (sha == null) throw new Exception("Failed to get file SHA for deletion");
        ...
        if (!deleteResponse.IsSuccessStatusCode)
            throw new Exception("GitHub delete failed");
    }
}
